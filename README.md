...
这个仓库用于演示如何从 Copernicus Climate Data Store 获取 ERA5 月度再分析数据，完成基础数据检查和清洗，并用 Dash 搭建本地交互式气候分析看板。

它的重点不是复杂建模，而是把“气象数据从哪里来、怎么自动下载、下载后怎么处理成可分析数据、如何进一步做趋势/空间/极端事件可视化”这条路径讲清楚。

## 仓库结构

| 路径 | 说明 |
|---|---|
| `download_era5_monthly.py` | ERA5 月度数据下载示例 |
| `inspect_netcdf.py` | 快速查看 NetCDF 文件变量和维度 |
| `process_era5_monthly.py` | 最小版清洗脚本：温度、风速、辐照、降水口径处理 |
| `dash_app/` | Dash 本地交互看板：趋势、空间、横截面、极端事件、ENSO、多变量等分析 |
...
可视化部分放在 `dash_app/`，这是一个本地 Dash 看板，不是 PowerCurve.cn 网站源码。
...
- 数据概览：年份、区域、变量和完整性
- 季节性分析：逐年对比、气候包络线
- 趋势分析：年度、季节、月度、距平和年际波动
- 空间分析：中国地图热力图和省份横截面对比
- 极端事件：百分位、z-score、变异系数等识别方式
- ENSO 关联：ONI、滞后相关、ENSO 阶段下的极端事件频次
- 多变量分析：相关性矩阵、散点回归、双地图对比
- 资源稳定性：均值-波动四象限和省份排序

出于数据合规和资产边界考虑，仓库不上传真实气象数据文件。

运行看板前，请在本地 `dash_app/data/` 下自行准备三个文件：

| 文件 | 用途 |
|---|---|
| `era5历史数据.xlsx` | Dash 主数据，省级月度 ERA5 指标 |
| `ONI.xlsx` | ENSO / ONI 指数 |
| `geo_plotly.json` | 中国省级地图底图 |

字段说明见 `dash_app/data/README.md`。如果替换成自己的数据，请保持字段口径与 `dash_app/main.py` 中读取的列名一致。
...
当前版本已经加入 Dash 本地看板，后续可以继续补充更轻量的示例数据、部署说明或专题图表。
